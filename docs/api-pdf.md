...
Ghi chú về `paddingLeft` / `paddingTop`

- `paddingLeft` *(number, optional, default = 0)*: dịch **vùng chữ ký** sang trái bằng cách trừ vào `x`.
  - Áp dụng cho cả 3 mode: `anchorPhrase`, `x/y` explicit, và default bottom-left.
- `paddingTop` *(number, optional, default = 0)*: dịch **vùng chữ ký** xuống dưới bằng cách trừ vào `y`.
  - Áp dụng cho cả 3 mode: `anchorPhrase`, `x/y` explicit, và default bottom-left.

Ví dụ: nếu bạn thấy box bị lệch *hơi sang phải* hoặc *hơi cao*, bạn có thể thử:
- tăng `paddingLeft` để kéo box sang trái
- tăng `paddingTop` để kéo box xuống dưới
