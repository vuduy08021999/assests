...
| `POST /api/ocr/text` | OCR text thô (Google Cloud Vision) |
...
### 5.6 POST /api/ocr/text

- **Mô tả:** OCR text thô bằng Google Cloud Vision (TEXT_DETECTION).
- **Lưu ý đồng bộ hợp đồng:** API key vẫn truyền qua body như các API khác.
  - `geminiApiKey`: **được dùng làm Google Cloud Vision API key** cho endpoint này.

- **Body bổ sung:**
  - `imageBase64` *(string, required)*: ảnh dạng base64. Server tự cắt prefix nếu bạn gửi `data:image/png;base64,...` hoặc thậm chí `image/png;base64,...`.

- **Body (tối thiểu) yêu cầu:**
```jsonc
{
  "geminiApiKey": "string",        // Google Vision API key
  "aiRequestTimeoutMs": 5000,
  "aiMaxRetries": 2,
  "imageBase64": "...",
  "requestId": "string?"
...
- **Response `data`:**
  | Field      | Kiểu                  | Ghi chú |
  |------------|-----------------------|--------|
  | `provider` | `GOOGLE_CLOUD_VISION`  | Hằng số |
  | `fullText` | string                | Text OCR trả về (có thể rỗng nếu không đọc được) |

**Ví dụ request:**
```json
{
  "geminiApiKey": "YOUR_GOOGLE_VISION_KEY",
  "requestId": "req-vision-001",
  "aiRequestTimeoutMs": 5000,
  "aiMaxRetries": 2,
  "imageBase64": "image/png;base64,iVBORw0KGgo..."
}
```
...
```json
{
  "status": "SUCCESS",
  "code": "OK",
  "message": "OCR Google Vision text success",
  "data": {
