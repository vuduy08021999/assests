...
import {
  ...
  OcrGoogleVisionProvider,
  OcrGoogleVisionTextRequestDto,
  OcrGoogleVisionTextResultDto,
  OcrIdCardRequestDto,
  OcrIdCardResultDto,
  OcrProvider,
...
export class OcrService implements IOcrService {
  constructor(
    ...
    private readonly googleVisionProvider: OcrGoogleVisionProvider,
  ) {}
  ...
  async processGoogleVisionTextOcr(request: OcrGoogleVisionTextRequestDto): Promise<OcrGoogleVisionTextResultDto> {
    const aiRequestTimeoutMs = Math.min(request.aiRequestTimeoutMs, MAX_GEMINI_TIMEOUT_MS);
    const aiMaxRetries = Math.min(request.aiMaxRetries, MAX_GEMINI_RETRIES);

    const payload = {
      imageBase64: request.imageBase64,
      // NOTE: kept as `geminiApiKey` in request body for consistency with existing APIs.
      // For this endpoint, it is used as the Google Cloud Vision API key.
      apiKey: request.geminiApiKey,
      aiRequestTimeoutMs,
      aiMaxRetries,
    };

    return this.googleVisionProvider.textDetection(payload);
  }
}
