...
const getVisibleSignFontCandidates = (): string[] => {
  const fromEnvRaw = stripWrappingQuotes(process.env.PDF_VISIBLE_SIGN_FONT_PATH || '');
  const candidates: string[] = [];

  if (fromEnvRaw) {
    candidates.push(fromEnvRaw);
    // If env path is relative, also try resolving from the current working directory.
    if (!path.isAbsolute(fromEnvRaw)) candidates.push(path.resolve(process.cwd(), fromEnvRaw));
  }

  // Project-local conventional location (optional; may not exist)
  candidates.push(path.resolve(process.cwd(), 'storage/fonts/NotoSans-Regular.ttf'));
  candidates.push(path.resolve(__dirname, '../../../storage/fonts/NotoSans-Regular.ttf'));

  // Common OS fonts
  if (process.platform === 'win32') {
    candidates.push('C:\\Windows\\Fonts\\arial.ttf');
    candidates.push('C:\\Windows\\Fonts\\ARIAL.TTF');
    candidates.push('C:\\Windows\\Fonts\\tahoma.ttf');
    ...
    candidates.push('/usr/share/fonts/opentype/noto/NotoSans-Regular.ttf');
  }
...
const resolveVisibleSignFontPath = (): string | null => {
  const candidates = getVisibleSignFontCandidates();
...
async function signPdfVisibleBase64(body: any): Promise<{ pdfBase64: string }> {
  ...
  // Visible-sign text must support Unicode (Vietnamese diacritics). StandardFonts
  // use WinAnsi encoding and will throw for characters like "Ạ".
  // Strategy:
  // - Always have a standard font (Helvetica) for ASCII-only text.
  // - For any non-ASCII text, require and embed a TTF/OTF font.
  const standardFont = await doc.embedFont(StandardFonts.Helvetica);
  const visibleFontPath = resolveVisibleSignFontPath();
  let unicodeFont: any | null = null;

  const needsUnicodeFont = (s: string) => /[^\x00-\x7F]/.test(s || '');
  ...
  const ensureUnicodeFont = async () => {
    ...
    if (!visibleFontPath) {
      throw new AppError(
        500,
        'SIGN_APPEARANCE_FAILED',
        'Failed to render visible signature text: Unicode characters require a TTF/OTF font. Configure PDF_VISIBLE_SIGN_FONT_PATH (e.g., to C:\\Windows\\Fonts\\arial.ttf) or place a Vietnamese-capable font at storage/fonts/NotoSans-Regular.ttf',
        { triedPaths: getVisibleSignFontCandidates() },
      );
    }
    ...
    try {
      ...
      const fontBytes = await fs.readFile(visibleFontPath);
      ...
      throw new AppError(
        500,
        'SIGN_APPEARANCE_FAILED',
        'Failed to load configured font for visible signature text. Ensure PDF_VISIBLE_SIGN_FONT_PATH points to a readable .ttf/.otf that supports Vietnamese.',
        { fontPath: visibleFontPath, error: msg },
      );
    }
  };
  ...
  try {
    ...
    for (let si = 0; si < signers.length; si++) {
      ...
      const paddingTop = typeof signer.paddingTop === 'number' ? signer.paddingTop : 0;
      ...
      if (signer.anchorPhrase && anchorPositions.has(si)) {
        ...
        y = Math.max(12, topOfBox - height - paddingTop);
      } else if (providedRect) {
        x = signer.x - paddingLeft;
        y = signer.y - paddingTop;
        width = signer.width;
      ...
      const hasUnicode = lines.some((l) => needsUnicodeFont(l));
      const font = hasUnicode ? await ensureUnicodeFont() : standardFont;
      ...
      for (const line of lines) {
        try {
          ...
          // If we are using a standard font and it can't encode Unicode, provide a clearer error.
          const msg = err?.message ?? String(err);
          if (/WinAnsi cannot encode/i.test(msg)) {
            // This can only happen when a standard font is used. Provide actionable guidance.
            throw new AppError(
              500,
              'SIGN_APPEARANCE_FAILED',
              'Failed to render visible signature text: Unicode characters require a TTF/OTF font. Configure PDF_VISIBLE_SIGN_FONT_PATH (e.g., to C:\\Windows\\Fonts\\arial.ttf) or place a Vietnamese-capable font at storage/fonts/NotoSans-Regular.ttf',
              { error: msg, triedPaths: getVisibleSignFontCandidates() },
            );
          }
          throw err;
        }
